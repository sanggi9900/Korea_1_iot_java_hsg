...
// == 자바에서의 타입 변환 ==
// 자료형 변환: 변수 & 자료형에서의 자동 형 변한, 강제 형 변환
// 클래스 타입 변환 : 자동, 강제 형 변환
// - 자료형 변환보다 가능한 범위가 좁음
// - 서로 상속 관계에 있는 클래스 사이에서만 변환 가능 (extends)
...
public class A_Class_Type {
	public static void main(String[] args) {
		...
		//childObject.ownMethod();   // - Error(The method ownMethod() is undefined for the type Parent)
		// >> 자동 형 변환의 경우, 해당 객체는 보존되면서
		//    사용은 부모 객체처럼 사용
		// >> 두 클래스 간에 공통된 특성이 아닌 경우 자식만의 기능 사용 x
		
		// 2. 이미 만들어진 자식 객체를 부모타입으로 변환
		Child c1 = new Child();
