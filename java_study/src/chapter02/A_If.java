...
//			조건이 거짓일때 실행될 코드
		...
		// == 조건문 예제 == 
		// : 함수 위치 구하기 문제
		// 1. 스캐너를 사용하여 x, y 별수에 데이터를 저장
		// 2. (x, y) 좌표가 1, 2, 3, 4 사분기 중 어느 위치에 있는지 검사
		// >> 조건문
		
		// 각 분기의 위치일 경우 functionsScope 변수에 몇 사분면인지 할당
...
//		int y = sc.nextInt();
//		
//		if(x > 0 && y > 0) System.out.println("결과: 1사분면입니다");
//		else if(x < 0 && y > 0) System.out.println("결과: 2사분면입니다");
//		else if(x < 0 && y < 0) System.out.println("결과: 3사분면입니다");
//		else if(x > 0 && y < 0) System.out.println("결과: 4사분면입니다");
//		else System.out.println("중앙입니다");
//		
//		sc.close();
		
		// == 풀이 ==
//		각 조건식에서 저장할 문자열값 (사분면의 위치)을 담을 변수 선언
		String functionScope = null; // 문자열 빈값인 변수 있다
